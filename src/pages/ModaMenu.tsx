...
export default function ModaMenu() {
  ...
  const { updatedPos, shouldUpdate, confirmUpdate, cancelUpdate } = useWatchPositionWithPrompt(position);
  ...
  const fetchWeatherForLocation = async (lat: number, lng: number) => {
    ...
    if (cached) {
      ...
      if (parsed.date === today && parsed.data?.lat === lat && parsed.data?.lng === lng) {
    ...
    const cityNoAccent = removeVietnameseTones(cityName);
    const weatherData = await fetchWeatherByCity(cityNoAccent);
    if (weatherData) {
      setWeather(weatherData);
      localStorage.setItem(
        WEATHER_STORAGE_KEY,
        JSON.stringify({ date: today, data: { lat, lng, city: cityName, weather: weatherData } })
      );
    } else {
  ...
  useEffect(() => {
    if (shouldUpdate && updatedPos) {
      Modal.confirm({
        ...
        content: 'Vị trí của bạn đã thay đổi hơn 300m. Bạn có muốn cập nhật lại thời tiết không?',
        onOk: () => {
